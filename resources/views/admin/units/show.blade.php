    ...
    {{ __('messages.back_to_units') }}
        ...
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                ...
                <div><strong>{{ __('messages.rent_price') }}:</strong> {{ number_format($unit->rent_price) }}
        ...
        @if ($unit->status === 'occupied' && $unit->latestContract && $unit->latestContract->tenant)
            <div class="bg-blue-50 rounded-lg shadow p-6 mb-6 border border-blue-200">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div><strong>{{ __('messages.tenant_name') }}:</strong> {{ $unit->latestContract->tenant->name }}
                    </div>
                    <div><strong>{{ __('messages.tenant_phone') }}:</strong>
                        {{ $unit->latestContract->tenant->phone ?? '-' }}</div>
                    <div><strong>{{ __('messages.contract_start') }}:</strong> {{ $unit->latestContract->start_date }}
                    </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @foreach ($unit->contracts->sortByDesc('start_date') as $contract)
                                <tr>
                                    ...
                                    <td class="px-4 py-2">{{ number_format($contract->rent_amount) }}

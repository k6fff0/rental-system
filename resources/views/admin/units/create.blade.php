        ...
        @if ($errors->any())
            <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-6 mb-6">
                <div class="flex items-start">
                    <div class="w-10 h-10 bg-red-500 rounded-lg flex items-center justify-center mr-3 rtl:ml-3 flex-shrink-0">
                        <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                    </div>
                    <div class="flex-1">
                        <h3 class="text-lg font-semibold text-red-800 dark:text-red-200 mb-2">{{ __('messages.validation_errors') }}</h3>
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2 space-y-2">
                            <label for="building_id" class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        {{-- رقم الوحدة --}}
                        <div class="space-y-2">
                            <label for="unit_number" class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        {{-- الطابق --}}
                        <div class="space-y-2">
                            <label for="floor" class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        {{-- نوع الوحدة --}}
                        <div class="space-y-2">
                            <label for="unit_type" class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300">
                            ...
                            <select name="unit_type" id="unit_type" 
                                ...
                                @foreach ($types as $type)
                                    <option value="{{ $type->value }}" {{ old('unit_type') === $type->value ? 'selected' : '' }}>
                        ...
                        {{-- السعر --}}
                        <div class="space-y-2">
                            <label for="rent_price" class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach ($statuses as $value => $config)
                                <label class="relative cursor-pointer">
                                    ...
                                    <div class="flex items-center p-4 bg-{{ $config['color'] }}-50 dark:bg-{{ $config['color'] }}-900/20 border-2 border-{{ $config['color'] }}-200 dark:border-{{ $config['color'] }}-800 rounded-xl transition-all duration-200 peer-checked:border-{{ $config['color'] }}-500 peer-checked:bg-{{ $config['color'] }}-100 dark:peer-checked:bg-{{ $config['color'] }}-900/40 hover:shadow-md">
                                        <div class="w-10 h-10 bg-{{ $config['color'] }}-500 rounded-lg flex items-center justify-center mr-3 rtl:ml-3 flex-shrink-0">
                                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $config['icon'] }}"/>
                                            </svg>
                                        </div>
                                        <div>
                                            <p class="font-medium text-{{ $config['color'] }}-800 dark:text-{{ $config['color'] }}-200">
                                                {{ __('messages.' . $config['label']) }}
                                            </p>
                                            <p class="text-sm text-{{ $config['color'] }}-600 dark:text-{{ $config['color'] }}-300">
                                                @if($value === 'available')
                                                    متاحة للإيجار
                                                @elseif($value === 'booked')
                                                    محجوزة من قبل عميل
                    ...
                    <div class="p-6">
                        <div class="space-y-2">
                            <label for="notes" class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300">
                                <svg class="w-4 h-4 mr-2 rtl:ml-2 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.getElementById('unitForm').addEventListener('submit', function(e) {
    ...
    if (rentPrice.value && parseFloat(rentPrice.value) <= 0) {
        ...
        if (!errorMsg) {
            errorMsg = document.createElement('p');
            errorMsg.className = 'error-message text-sm text-red-600 dark:text-red-400 flex items-center mt-1';
            errorMsg.innerHTML = '<svg class="w-4 h-4 mr-1 rtl:ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>يجب أن يكون السعر أكبر من صفر';
...
window.addEventListener('load', function() {
    ...
    if (draft) {
        ...
        for (let [key, value] of Object.entries(data)) {
            const input = document.getElementById(key) || document.querySelector(`input[name="${key}"]`) || document.querySelector(`select[name="${key}"]`) || document.querySelector(`textarea[name="${key}"]`);
            if (input) {
                if (input.type === 'radio') {

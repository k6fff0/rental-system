            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <label class="inline-flex items-center">
                        <input type="radio" name="status" value="available" class="h-4 w-4 text-green-500" {{ $unit->status == 'available' ? 'checked' : '' }}>
                        <span class="ml-2 text-gray-700">{{ __('messages.status_available') }}</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="status" value="maintenance" class="h-4 w-4 text-yellow-500" {{ $unit->status == 'maintenance' ? 'checked' : '' }}>
                        <span class="ml-2 text-gray-700">{{ __('messages.status_maintenance') }}</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="status" value="cleaning" class="h-4 w-4 text-indigo-500" {{ $unit->status == 'cleaning' ? 'checked' : '' }}>
                        <span class="ml-2 text-gray-700">{{ __('messages.status_cleaning') }}</span>
                    </label>
                </div>
            </div>

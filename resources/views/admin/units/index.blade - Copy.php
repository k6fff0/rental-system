...
<div class="p-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div onclick="filterBy('all')" class="cursor-pointer bg-white p-4 rounded-lg shadow border-l-4 border-gray-500 hover:bg-gray-100">
        ...
        <div onclick="filterBy('available')" class="cursor-pointer bg-white p-4 rounded-lg shadow border-l-4 border-blue-500 hover:bg-blue-50">
                ...
                <tr class="hover:bg-gray-50 transition" data-status="{{ $unit->status }}">
                    ...
                    <td class="px-4 py-3">
                        <form action="{{ route('admin.units.updateStatus', $unit->id) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            <select name="status" onchange="this.form.submit()" class="border rounded px-2 py-1 text-xs w-full">
                    ...
                    <td class="px-4 py-3 text-sm font-medium">
                        <div class="flex space-x-2">
                            <a href="{{ route('admin.units.edit', $unit->id) }}" class="text-blue-600 hover:text-blue-900" title="{{ __('messages.edit') }}">
                                ✏️
                            </a>
                            @if($unit->status === 'available')
                                <a href="{{ route('admin.contracts.create', ['unit_id' => $unit->id]) }}" class="text-green-600 hover:text-green-900" title="{{ __('messages.rent_unit') }}">
                                    ➕
                                </a>
                            @endif
                            <form action="{{ route('admin.units.destroy', $unit->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirm('{{ __('messages.confirm_delete') }}')" class="text-red-600 hover:text-red-900" title="{{ __('messages.delete') }}">
                                    🗑️
                                </button>
                            </form>
                        </div>

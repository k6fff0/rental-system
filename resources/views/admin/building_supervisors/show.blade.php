        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-4 sm:mb-6">
                <div class="flex flex-col space-y-4 lg:space-y-0 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex-1 min-w-0">
                        ...
                        <div class="{{ app()->getLocale() === 'ar' ? 'mr-11' : 'ml-11' }}">
                            <p class="text-xs sm:text-sm text-gray-600">
                                {{ __('messages.total_buildings') }}: <span
                                    class="font-semibold">{{ $buildings->total() ?? $buildings->count() }}</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 flex-shrink-0">
                        <div
                            class="inline-flex items-center px-3 py-2 bg-blue-50 border border-blue-200 text-blue-800 rounded-lg text-xs sm:text-sm font-medium">
                            <svg class="{{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }} h-4 w-4" fill="none"
                        ...
                        @if ($user->phone)
                            <div
                                class="inline-flex items-center px-3 py-2 bg-green-50 border border-green-200 text-green-800 rounded-lg text-xs sm:text-sm font-medium">
                                <svg class="{{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }} h-4 w-4" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @else
                            <div
                                class="inline-flex items-center px-3 py-2 bg-gray-50 border border-gray-200 text-gray-500 rounded-lg text-xs sm:text-sm font-medium">
                                <svg class="{{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }} h-4 w-4" fill="none"
                                    viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636m12.728 12.728L18 21L21 18m-7-7l-1-1" />
                                </svg>
            ...
            <div class="block lg:hidden space-y-3">
                @forelse($buildings as $building)
                    <div
                        class="bg-white rounded-lg border border-gray-200 shadow-sm p-4 hover:shadow-md transition-shadow duration-150">
                        <div class="flex items-start space-x-3 rtl:space-x-reverse">
                            <div
                                class="flex-shrink-0 h-12 w-12 rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center shadow-sm">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if ($building->address)
                                    <div class="flex items-start space-x-2 rtl:space-x-reverse mb-2">
                                        <svg class="h-4 w-4 text-gray-400 mt-0.5 flex-shrink-0" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col"
                                    class="px-6 py-3 {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }} text-xs font-medium text-gray-500 uppercase tracking-wider">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div
                                                class="flex-shrink-0 h-10 w-10 rounded-lg bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center {{ app()->getLocale() === 'ar' ? 'ml-4' : 'mr-4' }} shadow-sm">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <svg class="{{ app()->getLocale() === 'ar' ? 'ml-2' : 'mr-2' }} h-4 w-4 text-gray-400"
                ...
                @if ($buildings->hasPages())
                    <div class="bg-white px-6 py-3 border-t border-gray-200">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                            <div
                                class="text-xs sm:text-sm text-gray-700 text-center sm:{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                    ...
                    <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
                        <div class="text-center mb-3">
                            <span class="text-xs text-gray-600">
                                {{ __('messages.showing') }} {{ $buildings->firstItem() }} - {{ $buildings->lastItem() }}
                                {{ __('messages.of') }} {{ $buildings->total() }}
                            </span>
                        </div>

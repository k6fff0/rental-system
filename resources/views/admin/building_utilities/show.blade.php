    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center">
                <div
                    class="bg-blue-100 dark:bg-blue-900 p-3 rounded-full {{ app()->getLocale() === 'ar' ? 'ml-3' : 'mr-3' }}">
        ...
        @php
            ...
            $images = array_values(
                array_filter((array) $imagesArray, fn($img) => is_string($img) && !empty(trim($img))),
            );
        @endphp
                ...
                @foreach ($details as $key => $value)
                    @if ($value !== null && $value !== '')
                        <div class="flex flex-col sm:flex-row sm:items-start py-1">
                            <strong
                                class="w-full sm:w-1/3 text-gray-700 dark:text-gray-300 font-semibold">{{ __('messages.' . $key) }}:</strong>
                            <span
                                class="w-full sm:w-2/3 text-gray-900 dark:text-gray-100 mt-1 sm:mt-0">{{ $value }}</span>
                        </div>
                    @endif
                @endforeach
            </div>
            ...
            @if (count($images) > 0)
                <div class="pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div x-data="{
                        ...
                        @mouseleave="startAutoplay()" class="w-full max-w-lg mx-auto" {{-- يمكن تعديل max-width حسب الحاجة --}}
                        aria-labelledby="owner-id-images-heading">
                        {{-- حاوية الصورة الرئيسية للسلايدر --}}
                        <div
                            class="relative aspect-[4/3] sm:aspect-[16/10] overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-700">
                            <template x-for="(img, index) in images" :key="index">
                                <div x-show="current === index" x-cloak
                                    ...
                                    <a :href="'/storage/' + img" target="_blank" rel="noopener noreferrer"
                                        class="flex items-center justify-center w-full h-full">
                                        <img :src="'/storage/' + img"
                                            class="max-w-full max-h-full object-contain rounded-md p-1"
                                            {{-- p-1 لإضافة هامش بسيط حول الصورة داخل الإطار --}}
                            ...
                            <template x-if="count > 1">
                                <div class="absolute inset-0 flex justify-between items-center px-1 sm:px-2 z-10"
                                    role="navigation">
                                    <button @click="prev()" aria-label="{{ __('messages.previous_image') }}"
                                        ...
                                        <svg class="w-5 h-5 sm:w-6 sm:h-6 {{ app()->getLocale() === 'ar' ? 'transform rotate-180' : '' }}"
                                    ...
                                    <button @click="next()" aria-label="{{ __('messages.next_image') }}"
                                        class="bg-white/70 hover:bg-white dark:bg-gray-800/70 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-200 hover:text-black dark:hover:text-white rounded-full p-2 shadow-md transition-colors duration-200">
                                        <svg class="w-5 h-5 sm:w-6 sm:h-6 {{ app()->getLocale() === 'ar' ? 'transform rotate-180' : '' }}"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                            xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div class="flex justify-center mt-4 space-x-2 rtl:space-x-reverse" role="tablist">
                                <template x-for="(img, index) in images" :key="'dot-' + index">
                                    <button @click="goTo(index)"
                                        ...
                                        :aria-selected="current === index" role="tab"
                                        :class="{
                                            'bg-blue-600 dark:bg-blue-500 w-3 h-3': current === index,
                                            'bg-gray-300 dark:bg-gray-600 hover:bg-gray-400 dark:hover:bg-gray-500 w-2.5 h-2.5': current !==
                                                index
                                        }"
                                        class="rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800"></button>
                                </template>
                            </div>
                        </template>
                    </div>
                </div>
            @elseif(isset($buildingUtility->owner_id_image) &&
                    !empty(json_decode($buildingUtility->owner_id_image, true)) &&
                    count($images) === 0)
                {{-- حالة وجود بيانات صور ولكنها غير صالحة أو فارغة بعد الفلترة --}}
                <div class="pt-6 border-t border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-3" id="owner-id-images-heading">
                        {{ __('messages.owner_id_image') }}:</h3>
                    <p
                        class="text-gray-600 dark:text-gray-400 bg-yellow-50 dark:bg-yellow-700 border border-yellow-200 dark:border-yellow-600 p-3 rounded-md">
                        {{ __('messages.no_valid_images_found') }}</p>
                </div>
            @else
            ...
            <div class="pt-6 border-t border-gray-200 dark:border-gray-700 space-y-4 text-sm">
                @if ($buildingUtility->notes)
                    <div>
                        ...
                        <p class="text-gray-800 dark:text-gray-100 whitespace-pre-wrap mt-1">{{ $buildingUtility->notes }}
                        </p>
                    </div>
                @endif
                <div>
                    <strong
                        class="text-gray-700 dark:text-gray-300 font-semibold">{{ __('messages.created_at') }}:</strong>
                    <span
                        class="text-gray-800 dark:text-gray-100">{{ $buildingUtility->created_at->format('Y-m-d H:i A') }}</span>
                </div>
                <div>
                    <strong
                        class="text-gray-700 dark:text-gray-300 font-semibold">{{ __('messages.updated_at') }}:</strong>
                    <span
                        class="text-gray-800 dark:text-gray-100">{{ $buildingUtility->updated_at->format('Y-m-d H:i A') }}</span>
                </div>
            </div>

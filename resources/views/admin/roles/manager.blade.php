    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <form method="POST" action="{{ route('admin.role_manager.store') }}" class="bg-white shadow p-6 rounded-lg mb-6">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                <input type="text" name="name" required placeholder="{{ __('messages.enter_role_name') }}"
        ...
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach ($roles as $role)
                    <div
                        class="bg-white p-6 shadow rounded-xl border border-gray-200 flex flex-col justify-between relative group">

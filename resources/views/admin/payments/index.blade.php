                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-3">
                                    ...
                                    @endcan
                                    @can('delete payments')
                                    <form action="{{ route('admin.payments.destroy', $payment->id) }}" method="POST" onsubmit="return confirm('{{ __('messages.confirm_delete_payment') }}')">
                                        @csrf
                                        @method('DELETE')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('keyup', function(e) {
        if (e.key === 'Enter') {
            window.location.href = "{{ route('admin.payments.index') }}?search=" + encodeURIComponent(this.value);
        }
    });
});
</script>

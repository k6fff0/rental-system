            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div class="flex-1">
                    <div class="flex items-center space-x-4 rtl:space-x-reverse mb-4">
                        ...
                        <div>
                            <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white">{{ __('messages.building_details') }}</h1>
                    ...
                    <nav class="flex" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-2 rtl:space-x-reverse text-sm">
                            <li class="inline-flex items-center">
                                <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                            ...
                            <li>
                                <div class="flex items-center">
                                    <svg class="w-3 h-3 text-gray-400 mx-2" fill="none" stroke="currentColor" viewBox="0 0 6 10">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 9 4-4-4-4"/>
                                    </svg>
                                    <a href="{{ route('admin.buildings.index') }}" class="text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @php
                                $generalInfo = [
                                    ['key' => 'building_name', 'value' => $building->name, 'icon' => 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4'],
                                    ['key' => 'building_number', 'value' => $building->building_number, 'icon' => 'M7 20l4-16m2 16l4-16M6 9h14M4 15h14'],
                                    ['key' => 'address', 'value' => $building->address, 'icon' => 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z'],
                                    ['key' => 'municipality_number', 'value' => $building->municipality_number, 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z'],
                                    ['key' => 'units_count', 'value' => $building->units()->count(), 'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'],
                                    ['key' => 'created_at', 'value' => $building->created_at?->format('Y-m-d H:i A'), 'icon' => 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
                                ];
                            @endphp
                            
                            @foreach ($generalInfo as $info)
                                <div class="flex items-start p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600 hover:shadow-md transition-all duration-200">
                                    <div class="w-10 h-10 bg-green-100 dark:bg-green-900/20 rounded-lg flex items-center justify-center mr-3 rtl:ml-3 flex-shrink-0">
                        ...
                        @if (!empty($locationUrl) && filter_var($locationUrl, FILTER_VALIDATE_URL))
                            <div class="mt-6 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200 dark:border-blue-800">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center mr-3 rtl:ml-3">
                                            <svg class="w-5 h-5 text-blue-600 dark:text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            </svg>
                                        </div>
                                        <div>
                                            <p class="text-sm font-medium text-gray-600 dark:text-gray-400">{{ __('messages.location_url') }}</p>
                                            <p class="text-sm text-gray-500 dark:text-gray-500">موقع المبنى على الخريطة</p>
                                        </div>
                                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @php
                                $ownerInfo = [
                                    ['key' => 'owner_name', 'value' => $building->owner_name, 'icon' => 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z'],
                                    ['key' => 'owner_nationality', 'value' => $building->owner_nationality, 'icon' => 'M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 1H21l-3 6 3 6h-8.5l-1-1H5a2 2 0 00-2 2zm9-13.5V9'],
                                    ['key' => 'owner_id_number', 'value' => $building->owner_id_number, 'icon' => 'M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0a2 2 0 104 0m-5 8a2 2 0 100-4 2 2 0 000 4zm0 0c1.306 0 2.417.835 2.83 2M9 14a3.001 3.001 0 00-2.83 2M15 11h3m-3 4h2'],
                                    ['key' => 'owner_phone', 'value' => $building->owner_phone, 'icon' => 'M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z'],
                                ];
                            @endphp
                            
                            @foreach ($ownerInfo as $info)
                                <div class="flex items-start p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600 hover:shadow-md transition-all duration-200">
                                    <div class="w-10 h-10 bg-purple-100 dark:bg-purple-900/20 rounded-lg flex items-center justify-center mr-3 rtl:ml-3 flex-shrink-0">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @php
                                $financialInfo = [
                                    [
                                        'key' => 'rent_amount', 
                                        'value' => $building->rent_amount ? number_format($building->rent_amount, 2) . ' ' . __('messages.aed') : null,
                                        'icon' => 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1'
                                    ],
                                    [
                                        'key' => 'initial_renovation_cost', 
                                        'value' => $building->initial_renovation_cost ? number_format($building->initial_renovation_cost, 2) . ' ' . __('messages.aed') : null,
                            ...
                            @foreach ($financialInfo as $info)
                                <div class="flex items-start p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl border border-gray-200 dark:border-gray-600 hover:shadow-md transition-all duration-200">
                                    <div class="w-10 h-10 bg-orange-100 dark:bg-orange-900/20 rounded-lg flex items-center justify-center mr-3 rtl:ml-3 flex-shrink-0">
            ...
            <div class="xl:col-span-1 space-y-6">
                                ...
                                <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-20 transition-all duration-300 rounded-xl flex items-center justify-center">
                                    <a href="{{ asset('storage/' . $building->image) }}" 
                                       ...
                                       class="opacity-0 group-hover:opacity-100 bg-white dark:bg-gray-800 text-gray-800 dark:text-white px-4 py-2 rounded-lg shadow-lg transition-all duration-300 transform scale-75 group-hover:scale-100">
                                        <svg class="w-5 h-5 inline mr-2 rtl:ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            <div class="w-full h-64 bg-gray-100 dark:bg-gray-700 rounded-xl flex items-center justify-center">
                                <div class="text-center">
                                    ...
                                    <p class="text-gray-500 dark:text-gray-400">{{ __('messages.no_image_available') }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div class="bg-gradient-to-r from-indigo-500 to-blue-600 px-6 py-4">
                        ...
                        <p class="text-blue-100 text-sm mt-1">{{ __('messages.all_utilities_associated_with_this_building') }}</p>
                    </div>
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <div class="space-y-1 text-xs">
                                                    <div class="flex items-center justify-between">
                                                        ...
                                                        <span class="text-gray-900 dark:text-white font-medium">{{ $utility->value }}</span>
                                                    </div>
                                                    
                                                    @if($utility->owner_name)
                                                        <div class="flex items-center justify-between">
                                                            <span class="text-gray-500 dark:text-gray-400">المالك:</span>
                                                            <span class="text-gray-900 dark:text-white font-medium">{{ $utility->owner_name }}</span>
                                                        </div>
                                                    @endif
                                                    
                                                    @if($utility->notes)
                                                        <div class="mt-2">
                                                            <p class="text-gray-600 dark:text-gray-300 text-xs bg-gray-50 dark:bg-gray-700 p-2 rounded-lg">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        @php
                            $stats = [
                                ['label' => 'إجمالي الوحدات', 'value' => $building->units()->count(), 'color' => 'blue', 'icon' => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'],
                                ['label' => 'الوحدات المؤجرة', 'value' => $building->units()->whereHas('contracts', function($q) { $q->where('end_date', '>=', now()); })->count(), 'color' => 'green', 'icon' => 'M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z'],
                                ['label' => 'الوحدات الشاغرة', 'value' => $building->units()->whereDoesntHave('contracts', function($q) { $q->where('end_date', '>=', now()); })->count(), 'color' => 'red', 'icon' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z'],
                                ['label' => 'المرافق المسجلة', 'value' => $building->utilities()->count(), 'color' => 'purple', 'icon' => 'M13 10V3L4 14h7v7l9-11h-7z'],
                            ];
                        @endphp
                        
                        @foreach($stats as $stat)
                            <div class="flex items-center justify-between p-3 bg-{{ $stat['color'] }}-50 dark:bg-{{ $stat['color'] }}-900/20 rounded-lg border border-{{ $stat['color'] }}-200 dark:border-{{ $stat['color'] }}-800">
                                ...
                                <span class="text-xl font-bold text-{{ $stat['color'] }}-600 dark:text-{{ $stat['color'] }}-400">{{ $stat['value'] }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>

...
<style>
    ...
    @media (min-width: 1024px) { /* lg breakpoint */
        ...
        .details-grid.lg\:grid-cols-3 > div:nth-child(3n) { /* كل ثالث عنصر */
            border-right-width: 0;
            padding-right: 0;
        }
        .details-grid.lg\:grid-cols-3 > div:nth-child(3n+1) { /* كل أول عنصر في الصف الجديد */
             padding-left:0;
        }
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('admin.buildings.index') }}" class="ms-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ms-2 dark:text-gray-400 dark:hover:text-white">{{ __('messages.buildings') }}</a>
                        </div>
                    </li>
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">{{ $building->name }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
   ...
   class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 dark:focus:ring-offset-gray-800 transition-colors duration-150">
    ...
    <div class="bg-white dark:bg-gray-800 shadow-xl rounded-lg overflow-hidden">
        <div class="p-6 sm:p-8">
            ...
            <div class="mb-8">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-white mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">
                ...
                <div class="details-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 text-sm">
                    ...
                    <div class="flex justify-between items-start">
                        <strong class="text-gray-600 dark:text-gray-400 font-medium">{{ __('messages.' . $key) }}:</strong>
                        <span class="text-gray-800 dark:text-gray-200 text-right {{ app()->getLocale() === 'ar' ? 'text-left' : 'text-right' }}">{{ $value ?? '-' }}</span>
                    </div>
                    @endforeach
                    ...
                    <div class="flex justify-between items-center">
                        <strong class="text-gray-600 dark:text-gray-400 font-medium">{{ __('messages.location_url') }}:</strong>
                        @if (!empty($locationUrl) && filter_var($locationUrl, FILTER_VALIDATE_URL))
                            <a href="{{ $locationUrl }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 hover:underline transition-colors duration-150">
                                {{ __('messages.view_on_map') }}
                                <svg class="w-4 h-4 {{ app()->getLocale() === 'ar' ? 'mr-1 transform rotate-180' : 'ml-1' }}" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M12.586 4.586a2 2 0 112.828 2.828l-3 3a2 2 0 01-2.828 0l-4-4a2 2 0 012.828-2.828L8 7.172l2.586-2.586z" clip-rule="evenodd"></path><path fill-rule="evenodd" d="M6.414 11.414a1 1 0 10-1.414 1.414l3 3a1 1 0 001.414 0l4-4a1 1 0 10-1.414-1.414L9 12.586l-2.586-2.586z" clip-rule="evenodd"></path></svg>
                            </a>
                        @else
            ...
            <div>
                ...
                <div class="details-grid grid grid-cols-1 sm:grid-cols-2 gap-x-6 text-sm">
                     @php
                        $financialInfo = [
                            'rent_amount' => $building->rent_amount ? number_format($building->rent_amount, 2) . ' ' . __('messages.aed') : null,
                            'initial_renovation_cost' => $building->initial_renovation_cost ? number_format($building->initial_renovation_cost, 2) . ' ' . __('messages.aed') : null,
                        ];
                    @endphp
        ...
        <div class="bg-gray-50 dark:bg-gray-750 px-6 py-4 {{ app()->getLocale() === 'ar' ? 'text-left' : 'text-right' }} border-t border-gray-200 dark:border-gray-700"></div>
...
<div class="bg-white dark:bg-gray-800 rounded-xl shadow-md detail-card overflow-hidden">
    ...
    <div class="px-6 py-5 border-b section-divider">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <div class="flex-shrink-0 h-10 w-10 rounded-md bg-green-100 dark:bg-green-900 flex items-center justify-center">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                <thead class="bg-gray-50 dark:bg-gray-750">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('messages.utility_type') }}</th>
                        <th class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('messages.utility_value') }}</th>
                        <th class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('messages.owner_name') }}</th>
                        <th class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider {{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">{{ __('messages.notes') }}</th>
                        <th class="px-6 py-3 text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider {{ app()->getLocale() === 'ar' ? 'text-left' : 'text-right' }}">{{ __('messages.actions') }}</th>
                    </tr>
                </thead>
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @foreach ($building->utilities as $index => $utility)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300">{{ __('messages.' . $utility->type) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300">{{ $utility->value }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300">{{ $utility->owner_name ?? '-' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-gray-700 dark:text-gray-300 max-w-xs truncate" title="{{ $utility->notes }}">
                                {{ Str::limit($utility->notes, 35) }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap {{ app()->getLocale() === 'ar' ? 'text-left' : 'text-right' }} space-x-2 rtl:space-x-reverse">
                                {{-- 👁 View --}}

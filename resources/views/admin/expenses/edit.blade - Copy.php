    ...
    <div class="bg-white p-6 rounded-lg shadow">
        <form action="{{ route('admin.expenses.update', $expense->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    <label for="invoice_image" class="block mb-1 text-sm font-medium">{{ __('messages.add_new_invoice') }}</label>
            ...
            <div class="mt-8 flex flex-wrap items-center gap-4">
                ...
                @if ($expense->invoice_image)
                    <a href="{{ asset('storage/' . $expense->invoice_image) }}"
                       onclick="window.open(this.href, '_blank', 'width=800,height=600'); return false;"
                       style="background-color: #2563EB; color: white;"
                       class="px-4 py-2 text-sm font-medium rounded-md shadow hover:opacity-90 transition whitespace-nowrap">

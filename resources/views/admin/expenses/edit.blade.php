            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" class="form-input w-full" required>
                        <option value="furniture" {{ $expense->type === 'furniture' ? 'selected' : '' }}>{{ __('messages.furniture') }}</option>
                        <option value="painting" {{ $expense->type === 'painting' ? 'selected' : '' }}>{{ __('messages.painting') }}</option>
                        <option value="plumbing" {{ $expense->type === 'plumbing' ? 'selected' : '' }}>{{ __('messages.plumbing') }}</option>
                        <option value="electronics" {{ $expense->type === 'electronics' ? 'selected' : '' }}>{{ __('messages.electronics') }}</option>
                        <option value="other" {{ $expense->type === 'other' ? 'selected' : '' }}>{{ __('messages.other') }}</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="building_id" id="building_id" class="form-input w-full" required>
                        @foreach ($buildings as $building)
                            <option value="{{ $building->id }}" {{ $expense->building_id == $building->id ? 'selected' : '' }}>
                                {{ $building->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="unit_id" id="unit_id" class="form-input w-full">
                        ...
                        @foreach ($units as $unit)
                            <option value="{{ $unit->id }}" {{ $expense->unit_id == $unit->id ? 'selected' : '' }}>
                                {{ $unit->unit_number }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" class="form-input w-full" step="0.01" required value="{{ $expense->amount }}">
                </div>

                <div>
                    <label for="expense_date" class="block mb-1 text-sm font-medium">{{ __('messages.date') }} *</label>
                    <input type="date" name="expense_date" id="expense_date" class="form-input w-full" required value="{{ $expense->expense_date }}">
                </div>

                <div>
                    <label for="invoice_images[]" class="block mb-1 text-sm font-medium">{{ __('messages.add_new_invoice') }}</label>
                    <input type="file" name="invoice_images[]" id="invoice_images" accept="image/*" class="form-input w-full" multiple>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach ($expense->images as $image)
                            <div class="bg-gray-50 p-3 rounded shadow border flex flex-col items-center">
                                <img src="{{ asset('storage/' . $image->image_path) }}" alt="Invoice Image" class="w-full h-auto max-h-48 object-contain rounded">

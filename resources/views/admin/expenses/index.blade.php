    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-2">{{ __('messages.total_expenses') }}: {{ number_format($totalExpenses, 2) }} {{ __('messages.currency') }}</p>
            </div>
            ...
            <form method="GET" action="{{ route('admin.expenses.index') }}">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="building_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm rounded-md">
                    ...
                    <div>
                        ...
                        <select name="type" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-red-500 focus:border-red-500 sm:text-sm rounded-md">
                            <option value="">{{ __('messages.all') }}</option>
                    ...
                    <div class="flex items-end">
                        <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            <svg xmlns="http://www.w3.org/2000/svg" class="-ml-0 mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if ($expenses->isEmpty())
            <div class="bg-white p-8 rounded-xl shadow-md text-center">
                ...
                <div class="mt-6">
                    <a href="{{ route('admin.expenses.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.type') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.building') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.unit') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.amount') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.date') }}</th>
                                <th class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('messages.description') }}</th>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ __('messages.' . $expense->type) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $expense->unit->unit_number ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-semibold text-red-600">{{ number_format($expense->amount, 2) }} {{ __('messages.currency') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end space-x-2 rtl:space-x-reverse">
                                        <a href="{{ route('admin.expenses.edit', $expense->id) }}" class="text-blue-600 hover:text-blue-900">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </a>
                                        <form action="{{ route('admin.expenses.destroy', $expense->id) }}" method="POST" onsubmit="return confirm('{{ __('messages.confirm_delete') }}')">
                                            @csrf
                                            @method('DELETE')
                ...
                <div class="bg-white p-4 rounded-lg shadow border-l-4 border-red-500">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-gray-900">{{ number_format($totalExpenses, 2) }} {{ __('messages.currency') }}</p>
                        </div>
                ...
                <div class="bg-white p-4 rounded-lg shadow border-l-4 border-blue-500">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-gray-900">{{ number_format($thisMonthExpenses, 2) }} {{ __('messages.currency') }}</p>
                        </div>
                ...
                <div class="bg-white p-4 rounded-lg shadow border-l-4 border-green-500">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-gray-900">{{ number_format($lastMonthExpenses, 2) }} {{ __('messages.currency') }}</p>
                        </div>
                ...
                <div class="bg-white p-4 rounded-lg shadow border-l-4 border-yellow-500">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-semibold text-gray-900">{{ number_format($averageMonthlyExpenses, 2) }} {{ __('messages.currency') }}</p>
                        </div>
                    ...
                    <div class="p-4">
                        <div class="flex items-start justify-between">
                            <div class="flex items-center">
                                ...
                                <div class="ml-3">
                                    <h3 class="text-sm font-medium text-gray-900">{{ __('messages.' . $expense->type) }}</h3>
                        ...
                        <div class="mt-3">
                            <div class="text-sm text-gray-600">
                                <p><strong>{{ __('messages.building') }}:</strong> {{ $expense->building->name ?? '-' }}</p>
                                <p><strong>{{ __('messages.unit') }}:</strong> {{ $expense->unit->unit_number ?? '-' }}</p>
                                @if ($expense->description)
                                <p class="mt-1"><strong>{{ __('messages.description') }}:</strong> <span class="line-clamp-2">{{ $expense->description }}</span></p>
                                @endif
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 flex justify-end space-x-3 rtl:space-x-reverse">
                        <a href="{{ route('admin.expenses.edit', $expense->id) }}" class="inline-flex items-center px-3 py-1 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            {{ __('messages.edit') }}
                        </a>
                        <form action="{{ route('admin.expenses.destroy', $expense->id) }}" method="POST" onsubmit="return confirm('{{ __('messages.confirm_delete') }}')">
                            @csrf

        ...
        <div class="bg-white dark:bg-gray-800 shadow-xl rounded-2xl overflow-hidden ring-1 ring-gray-200 dark:ring-gray-700">
            ...
            <div class="p-6 space-y-6">
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->unit->unit_number ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('messages.building') }}</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->unit->building->name ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('messages.unit_type') }}</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ __('messages.unit_type_' . $booking->unit->unit_type) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                        ...
                        <div class="space-y-2 text-sm">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->user->name ?? '-' }}</span>
                            </div>
                            @can('view booking owner')
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('messages.email') }}</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->user->email ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('messages.phone') }}</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->user->phone ?? '-' }}</span>
                            </div>
                            @endcan
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        <div class="flex justify-between items-center bg-white dark:bg-gray-600 p-3 rounded-lg">
                            ...
                            <div class="text-right">
                                <div class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->start_date->format('Y-m-d') }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ __('messages.to') }}</div>
                                <div class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->end_date->format('Y-m-d') }}</div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->created_at->format('Y-m-d H:i') }}</span>
                            </div>
                            @if($booking->tentative_at)
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('messages.tentative_at') }}</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->tentative_at->format('Y-m-d H:i') }}</span>
                            </div>
                            @endif
                            @if($booking->confirmed_at)
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('messages.confirmed_at') }}</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->confirmed_at->format('Y-m-d H:i') }}</span>
                            </div>
                            @endif
                            @if($booking->auto_expire_at)
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('messages.auto_expire_at') }}</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->auto_expire_at->format('Y-m-d H:i') }}</span>
                            </div>
                            @endif
                            @if($booking->cancelled_at)
                            <div class="flex justify-between">
                                <span class="text-gray-600 dark:text-gray-400">{{ __('messages.cancelled_at') }}</span>
                                <span class="font-medium text-gray-800 dark:text-gray-200">{{ $booking->cancelled_at->format('Y-m-d H:i') }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <form action="{{ route('admin.bookings.confirm', $booking) }}" method="POST" class="flex-1">
                        @csrf
                        <button type="submit"

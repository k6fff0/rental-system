    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8"
        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
                ...
                <form method="POST"
                    action="{{ isset($specialty) ? route('admin.technicians.specialties.update', $specialty->id) : route('admin.technicians.specialties.store') }}"
                    class="space-y-6">
                    @csrf
                    @if (isset($specialty))
                        @method('PUT')
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label
                                class="flex items-center gap-3 p-4 border border-gray-200 rounded-xl hover:bg-gray-50 cursor-pointer transition-colors duration-200 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                                <input type="radio" name="is_main" value="1"
                                    {{ old('is_main', $specialty->is_main ?? 1) == 1 ? 'checked' : '' }}
                                    class="text-blue-600 focus:ring-blue-500">
                                <div>
                                    <span class="block font-medium text-gray-800">{{ __('messages.main_specialty') }}</span>
                                    <span
                                        class="block text-xs text-gray-500 mt-1">{{ __('messages.main_specialty_desc') }}</span>
                                </div>
                            </label>
                            <label
                                class="flex items-center gap-3 p-4 border border-gray-200 rounded-xl hover:bg-gray-50 cursor-pointer transition-colors duration-200 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                                <input type="radio" name="is_main" value="0"
                                    {{ old('is_main', $specialty->is_main ?? 1) == 0 ? 'checked' : '' }}
                                    class="text-blue-600 focus:ring-blue-500">
                                <div>
                                    <span class="block font-medium text-gray-800">{{ __('messages.sub_specialty') }}</span>
                                    <span
                                        class="block text-xs text-gray-500 mt-1">{{ __('messages.sub_specialty_desc') }}</span>
                                </div>
                            </label>
                        </div>
                        @error('is_main')
                    ...
                    <div id="parent_section" class="transition-all duration-300">
                        ...
                        <select name="parent_id" id="parent_id"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('parent_id') border-red-500 @enderror">
                            <option value="">— {{ __('messages.select_main_specialty') }} —</option>
                            @foreach ($mainSpecialties as $main)
                                <option value="{{ $main->id }}"
                                    {{ old('parent_id', $specialty->parent_id ?? '') == $main->id ? 'selected' : '' }}>
                                    {{ $main->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 pt-4">
                        <a href="{{ route('admin.technicians.specialties.index') }}"
                            class="px-6 py-3 bg-white border border-gray-200 text-gray-700 font-medium rounded-xl hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 text-center">
                            {{ __('messages.cancel') }}
                        </a>
                        <button type="submit"
                            class="px-6 py-3 bg-blue-600 text-white font-medium rounded-xl hover:bg-blue-700 transition-all duration-200 shadow-sm hover:shadow-md flex items-center justify-center gap-2">

...
@push('styles')
    <style>
        ...
        .header-modern h1 {
            ...
            /* أثقل للعنوان */
            color: #212529;
            /* لون داكن أكثر احترافية */
            margin: 0;
        }

        .header-modern p {
            color: #6c757d;
            /* لون رمادي ناعم للوصف */
            margin-top: 8px;
            font-size: 1.05rem;
        }
        ...
        .button-modern {
            ...
            /* ظل للأزرار */
            width: 100%;
            /* اجعل الزر يأخذ كامل العرض */
            max-width: 350px;
            /* حدد نفس أقصى عرض لمدخل البحث */
        }
        ...
        .sub-specialty-toggle {
            ...
            gap: 5px;
            margin-top: 15px;
            /* مسافة بين العنوان الفرعي والزر */
        ...
        .sub-specialties-grid {
            ...
            /* 200px الحد الأدنى للعرض */
            gap: 10px;
            /* مسافة أقل بين التخصصات الفرعية */
            padding-top: 15px;
            /* مسافة بين زر العرض والشبكة */
        }

        .sub-specialty-chip {
            background-color: #e9f0f9;
            /* لون خلفية خفيف للـ chip */
            color: #0056b3;
            padding: 8px 12px;
            border-radius: 20px;
            /* حواف دائرية للـ chip */
            font-size: 0.9rem;
            display: flex;
            align-items: center;
            justify-content: space-between;
        ...
        .chip-action-icon {
            ...
            transition: color 0.2s ease;
            font-size: 0.85rem;
            /* حجم أصغر للأيقونات داخل الـ chip */
        }
        ...
        <div id="specialtiesContainer">
            @forelse($specialties->where('is_main', true) as $main)
                <div class="specialty-card-modern">
                        ...
                        <div class="mt-4">
                            <button type="button" class="sub-specialty-toggle"
                                ...
                                <span class="text-gray-700">
                                    ...
                                    <span
                                        class="bg-gray-100 text-gray-600 text-xs px-2 py-0.5 rounded-full mr-1 {{ app()->getLocale() === 'ar' ? 'mr-1' : '' }}">{{ $subSpecialties->count() }}</span>
                                </span>
                                <svg class="w-4 h-4 text-blue-600 transition-transform duration-300 transform {{ app()->getLocale() === 'ar' ? 'rotate-180' : '' }}"
                    ...
                    @else
                        <div
                            class="mt-4 p-4 bg-gray-50 rounded text-center text-gray-500 text-sm border border-dashed border-gray-200">
                ...
                forms.forEach(form => {
                    form.addEventListener('submit', function() {
                        ...
                        if (submitBtn) {
                            ...
                            if (submitBtn.classList.contains('action-icon-button') || submitBtn
                                .classList.contains('chip-action-icon')) {
                                submitBtn.innerHTML = `

...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
                    ...
                    <div class="flex-1 w-full">
                        ...
                        <div class="relative">
                            <input type="text" name="search" id="search_filter" value="{{ request('search') }}"
                                   ...
                                   autocomplete="off">
                            <div id="search_results" class="absolute z-10 mt-1 w-full bg-white shadow-lg rounded-lg hidden max-h-60 overflow-auto"></div>
                        </div>
                    </div>
                    <button type="submit" 
                ...
                @forelse ($technicians as $tech)
                    <div class="border-b border-gray-100 p-6 hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <div class="mb-4">
                            ...
                            <p class="font-medium text-gray-800">{{ $tech->mainSpecialty->name ?? __('messages.no_specialty') }}</p>
                        </div>
            ...
            <div class="hidden md:block overflow-x-auto">
                <table class="w-full divide-y divide-gray-200"> <!-- تغيير من min-w-full إلى w-full -->
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-8 py-4 text-start text-xs font-bold text-gray-500 uppercase tracking-wider"> <!-- زيادة padding-x من 6 إلى 8 -->
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200">
                                <td class="px-8 py-4 whitespace-nowrap"> <!-- إضافة whitespace-nowrap -->
                                    <div class="flex items-center gap-4">
                                       <div class="w-10 h-10 rounded-full overflow-hidden bg-gray-100 border border-gray-300 shadow-sm">
    @if ($tech->photo_url)
        <img src="{{ asset('storage/' . $tech->photo_url) }}" alt="{{ $tech->name }}" class="object-cover w-full h-full">
    @else
        <div class="w-full h-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold">
    ...
    @endif
                                ...
                                <td class="px-8 py-4 whitespace-nowrap">
                                    @php
                                        $statusConfig = [
                                            'available' => ['bg' => 'bg-green-100', 'text' => 'text-green-800', 'dot' => 'bg-green-400'],
                                            'busy' => ['bg' => 'bg-yellow-100', 'text' => 'text-yellow-800', 'dot' => 'bg-yellow-400'],
                                            'unavailable' => ['bg' => 'bg-red-100', 'text' => 'text-red-800', 'dot' => 'bg-red-400'],
                                        ];
                                        $status = $tech->technician_status ?? 'unavailable';
                                ...
                                <td class="px-8 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('admin.technicians.show', $tech->id) }}"
                                           class="inline-flex items-center px-4 py-2 bg-blue-100 text-blue-700 font-medium rounded-lg hover:bg-blue-200 transition-colors duration-200 text-sm"> <!-- تغيير حجم النص من xs إلى sm -->

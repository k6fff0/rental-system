...
<div class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <form action="{{ route('admin.permissions.update', $permission->id) }}" method="POST" class="bg-white shadow rounded-xl p-6">
        @csrf
        @method('PUT')

        {{-- اسم الصلاحية --}}
        <div class="mb-6">
            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.permission_name') }}</label>

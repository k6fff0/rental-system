        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div class="{{ app()->getLocale() === 'ar' ? 'text-right' : 'text-left' }}">
                <h1 class="text-2xl md:text-3xl font-bold text-gray-800 flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 me-2 text-indigo-600" fill="none"
                ...
                <div class="mb-4">
                    <label for="tenant_search" class="block text-sm font-medium text-gray-700 mb-2">
                        <span class="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 me-1 text-blue-500" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="relative">
                        ...
                        <div id="tenant_results"
                            class="absolute z-10 bg-white border border-gray-200 w-full rounded-lg mt-1 hidden max-h-60 overflow-y-auto text-sm shadow-lg">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="relative mt-1 rounded-md shadow-sm">
                            ...
                            <div
                                class="absolute inset-y-0 {{ app()->getLocale() === 'ar' ? 'left-0' : 'right-0' }} flex items-center pr-3 pointer-events-none">
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="mt-1 flex items-center">
                            <label for="contract_file" class="cursor-pointer w-full">
                                <div
                                    class="flex items-center justify-between bg-white border border-gray-300 rounded-lg shadow-sm px-4 py-2 hover:bg-gray-50">
    ...
    <script>
            ...
            input.addEventListener('input', function() {
                ...
                fetch(`/admin/api/tenants/search?q=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        if (data.length === 0) {
                            resultsBox.innerHTML =
                                '<div class="p-3 text-center text-gray-500 bg-gray-50">{{ __('messages.no_results') }}</div>';
                        } else {
        ...
        buildingSelect.addEventListener('change', function() {
            ...
            unitSelect.innerHTML =
                `<option>{{ app()->getLocale() === 'ar' ? 'جاري التحميل...' : 'Loading...' }}</option>`;

            if (!buildingId) {
                unitSelect.innerHTML =
                    `<option value="">{{ app()->getLocale() === 'ar' ? 'اختر المبنى أولاً' : 'Choose building first' }}</option>`;
                return;
            }
            ...
            fetch(`/admin/api/buildings/${buildingId}/available-units`)
                ...
                .then(units => {
                    if (units.length === 0) {
                        unitSelect.innerHTML =
                            `<option value="">{{ app()->getLocale() === 'ar' ? 'لا توجد وحدات متاحة' : 'No available units' }}</option>`;
                    } else {
                        unitSelect.innerHTML =
                            `<option value="">{{ app()->getLocale() === 'ar' ? 'اختر الوحدة' : 'Choose unit' }}</option>`;
                        units.forEach(unit => {
                            unitSelect.innerHTML +=

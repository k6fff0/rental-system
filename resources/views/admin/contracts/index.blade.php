            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach ($contracts as $contract)
                        <tr class="hover:bg-gray-50 transition" data-status="{{ $contract->status }}">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <span class="inline-block px-2 py-1 rounded-full text-xs font-semibold
                                    {{ now()->gt($contract->end_date) ? 'bg-red-100 text-red-800' : (now()->diffInDays($contract->end_date) <= 30 ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800') }}">
                                    {{ now()->gt($contract->end_date) ? __('messages.expired') : (now()->diffInDays($contract->end_date) <= 30 ? __('messages.expiring_soon') : __('messages.active')) }}
                                </span>
                            </td>

...
@section('content')
    <div class="max-w-3xl mx-auto py-6 sm:px-6 lg:px-8" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <form action="{{ route('admin.maintenance_requests.update', $maintenance->id) }}" method="POST"
            enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-lg shadow">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" value="{{ $maintenance->creator->name ?? '-' }}" disabled
                        class="mt-1 block w-full bg-gray-100 border-gray-300 rounded-md shadow-sm text-sm">
                </div>
            </div>
            ...
            <div>
                ...
                @if ($maintenance->image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $maintenance->image) }}" class="h-32 w-auto rounded shadow border"
                            alt="Image">
                    </div>
                @endif

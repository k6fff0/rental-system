...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
    ...
    {{-- 🔧 العنوان --}}
    <div class="bg-gradient-to-r from-blue-500 to-blue-300 text-white p-6 rounded-lg shadow-md mb-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <a href="{{ route('admin.maintenance_requests.create') }}" class="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-md text-sm font-semibold shadow-md transition duration-300 whitespace-nowrap flex items-center gap-2">
    ...
    <div class="bg-white p-1 rounded-lg shadow-md mb-6 overflow-x-auto">
        <div class="flex space-x-1 min-w-max">
            @php
                $statusTabs = [
                    'all' => ['color' => 'bg-gray-100 hover:bg-gray-200', 'active' => 'bg-gray-300', 'text' => 'text-gray-800'],
                    'new' => ['color' => 'bg-yellow-100 hover:bg-yellow-200', 'active' => 'bg-yellow-300', 'text' => 'text-yellow-800'],
                    'in_progress' => ['color' => 'bg-blue-100 hover:bg-blue-200', 'active' => 'bg-blue-300', 'text' => 'text-blue-800'],
                    'completed' => ['color' => 'bg-green-100 hover:bg-green-200', 'active' => 'bg-green-300', 'text' => 'text-green-800'],
                    'rejected' => ['color' => 'bg-red-100 hover:bg-red-200', 'active' => 'bg-red-300', 'text' => 'text-red-800'],
                    'delayed' => ['color' => 'bg-orange-100 hover:bg-orange-200', 'active' => 'bg-orange-300', 'text' => 'text-orange-800'],
                    'waiting_materials' => ['color' => 'bg-purple-100 hover:bg-purple-200', 'active' => 'bg-purple-300', 'text' => 'text-purple-800'],
                    'customer_unavailable' => ['color' => 'bg-pink-100 hover:bg-pink-200', 'active' => 'bg-pink-300', 'text' => 'text-pink-800'],
                    'other' => ['color' => 'bg-gray-100 hover:bg-gray-200', 'active' => 'bg-gray-300', 'text' => 'text-gray-800'],
                ];
            @endphp
            ...
            @foreach(['new','in_progress','completed','rejected','delayed','waiting_materials','customer_unavailable','other'] as $status)
                ...
                <a href="{{ request()->fullUrlWithQuery(['status' => $status]) }}" 
                   class="px-4 py-2 rounded-md text-sm font-medium transition-all duration-200 whitespace-nowrap flex items-center gap-2 
                    ...
                    <span class="bg-white {{ $statusTabs[$status]['text'] }} text-xs px-2 py-0.5 rounded-full ml-1">{{ $statusCounts[$status] }}</span>
                </a>
                @endif
            @endforeach
        </div>
    ...
    <div class="bg-white p-4 rounded-lg shadow-md mb-6">
        <form id="filterForm" method="GET" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-center">
            ...
            {{-- 🔢 بحث برقم الغرفة --}}
            <div class="relative">
                <label class="block text-xs font-medium text-gray-500 mb-1">{{ __('messages.search_by_unit_number') }}</label>
                <div class="relative">
                    <input 
                        type="text" 
                        name="unit_number" 
            ...
            @forelse($requests as $request)
                ...
                <div class="border-b p-4 hover:bg-gray-50">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <span class="px-2 py-1 rounded-full text-xs {{ $statusTextColors[$request->status] ?? 'bg-gray-100 text-gray-800' }}">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('admin.maintenance_requests.show', $request->id) }}" class="bg-gray-100 px-3 py-1 rounded text-xs hover:bg-gray-200 flex items-center gap-1">
                        ...
                        <a href="{{ route('admin.maintenance_requests.edit', $request->id) }}" class="bg-blue-100 text-blue-800 px-3 py-1 rounded text-xs hover:bg-blue-200 flex items-center gap-1">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @if ($request->image)
                            <a href="{{ asset('storage/' . $request->image) }}" target="_blank" class="bg-indigo-100 text-indigo-800 px-3 py-1 rounded text-xs hover:bg-indigo-200 flex items-center gap-1">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form method="POST" action="{{ route('admin.maintenance_requests.update_status', $request->id) }}" class="mt-3">
                        @csrf
                        @method('PUT')
                        <select name="status" onchange="this.form.submit()" class="w-full text-xs border rounded px-2 py-1 pr-6 focus:ring-2 focus:ring-blue-500 appearance-none bg-white">
                            @foreach (['new','in_progress','completed','rejected','delayed','waiting_materials','customer_unavailable','other'] as $status)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center gap-2">
                                    <span class="w-3 h-3 rounded-full {{ $colors[$request->status] ?? 'bg-gray-300' }}" title="{{ __('messages.status_' . $request->status) }}"></span>
                                    <span class="px-2 py-1 rounded-full text-xs {{ $statusColors[$request->status] ?? 'bg-gray-100 text-gray-800' }}">
                                        {{ __('messages.status_' . $request->status) }}
                                    </span>
                                </span>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap gap-1">
                                    <a href="{{ route('admin.maintenance_requests.show', $request->id) }}" class="bg-gray-100 px-2 py-1 rounded text-xs hover:bg-gray-200 flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                        </svg>
                                    ...
                                    <a href="{{ route('admin.maintenance_requests.edit', $request->id) }}" class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs hover:bg-blue-200 flex items-center gap-1">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                        </svg>
                                        {{ __('messages.edit') }}
                                    ...
                                    @if ($request->image)
                                        <a href="{{ asset('storage/' . $request->image) }}" target="_blank" class="bg-indigo-100 text-indigo-800 px-2 py-1 rounded text-xs hover:bg-indigo-200 flex items-center gap-1">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                            </svg>
                                            {{ __('messages.image') }}
                            ...
                            <td class="px-4 py-3">
                                <form method="POST" action="{{ route('admin.maintenance_requests.update_status', $request->id) }}" class="relative">
                                    @csrf
                                    @method('PUT')
                                    <select name="status" onchange="this.form.submit()" class="w-full text-xs border rounded px-2 py-1 pr-6 focus:ring-2 focus:ring-blue-500 appearance-none bg-white">
                                        @foreach (['new','in_progress','completed','rejected','delayed','waiting_materials','customer_unavailable','other'] as $status)

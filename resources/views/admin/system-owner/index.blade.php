...
@section('content')
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8" dir="{{ app()->getLocale() === 'ar' ? 'rtl' : 'ltr' }}">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            @foreach ([['title' => 'المستخدمين', 'count' => $usersCount, 'icon' => 'users', 'color' => 'blue', 'route' => 'admin.users.index'], ['title' => 'المباني', 'count' => $buildingsCount, 'icon' => 'building', 'color' => 'green', 'route' => 'admin.buildings.index'], ['title' => 'الوحدات', 'count' => $unitsCount, 'icon' => 'home', 'color' => 'purple', 'route' => 'admin.units.index'], ['title' => 'العقود', 'count' => $contractsCount, 'icon' => 'file-contract', 'color' => 'yellow', 'route' => 'admin.contracts.index']] as $stat)
            ...
            <div class="bg-white dark:bg-gray-800 p-5 rounded-lg shadow">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('admin.roles.index') }}"
                        class="text-sm text-blue-600 dark:text-blue-400 hover:underline">إدارة الكل</a>
                </div>
                    ...
                    @foreach ($roles as $role)
                        <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded">
                            ...
                            <a href="{{ route('admin.roles.edit', $role->id) }}"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline">تعديل</a>
                        </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 p-5 rounded-lg shadow">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <form method="POST" action="{{ route('admin.backup.create') }}" class="contents">
                        @csrf
                        <button type="submit"
                            class="flex items-center justify-center gap-2 p-3 bg-blue-600 hover:bg-blue-700 text-white rounded transition">
                    ...
                    <button onclick="showRestoreModal()"
                        class="flex items-center justify-center gap-2 p-3 bg-green-600 hover:bg-green-700 text-white rounded transition">
                    ...
                    <form method="POST" action="{{ route('admin.backup.download') }}" class="contents">
                        @csrf
                        <button type="submit"
                            class="flex items-center justify-center gap-2 p-3 bg-purple-600 hover:bg-purple-700 text-white rounded transition">
                    ...
                    <form method="POST" action="{{ route('admin.backup.clean') }}" class="contents">
                        @csrf
                        <button type="submit"
                            class="flex items-center justify-center gap-2 p-3 bg-red-600 hover:bg-red-700 text-white rounded transition">
            ...
            <div class="bg-white dark:bg-gray-800 p-5 rounded-lg shadow">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <a href="{{ route('admin.settings.edit') }}"
                        class="text-sm text-blue-600 dark:text-blue-400 hover:underline">تعديل الكل</a>
                </div>

                <div class="space-y-3">
                    @foreach ([['key' => 'app_name', 'label' => 'اسم التطبيق', 'value' => settings('app_name', config('app.name'))], ['key' => 'app_logo', 'label' => 'شعار النظام', 'value' => settings('app_logo') ? 'مضبوط' : 'غير مضبوط'], ['key' => 'primary_color', 'label' => 'اللون الأساسي', 'value' => settings('primary_color', '#3b82f6')], ['key' => 'maintenance_mode', 'label' => 'وضع الصيانة', 'value' => settings('maintenance_mode', false) ? 'مفعل' : 'معطل']] as $setting)
                        <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded">
                            <div>
                            ...
                            <a href="{{ route('admin.settings.edit', ['setting' => $setting['key']]) }}"
                                class="text-sm text-blue-600 dark:text-blue-400 hover:underline">تعديل</a>
                        </div>
                    @endforeach
                </div>
            ...
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex gap-2">
                    <form method="POST" action="{{ route('admin.logs.clear') }}" class="contents">
                        @csrf
                        <button type="submit" onclick="return confirm('هل أنت متأكد من رغبتك في مسح جميع سجلات النظام؟')"
                            class="text-sm bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200 px-3 py-1 rounded hover:bg-red-200 dark:hover:bg-red-800 transition">
                            مسح السجلات
                        </button>
                    </form>
                    <a href="{{ route('admin.logs.download') }}"
                        class="text-sm bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 px-3 py-1 rounded hover:bg-blue-200 dark:hover:bg-blue-800 transition">
                        تحميل السجلات
                    </a>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 p-5 rounded-lg shadow mb-8">
            <div class="flex items-center justify-between mb-4">
                ...
                <button onclick="showContractModal()"
                    class="text-sm bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 px-3 py-1 rounded hover:bg-green-200 dark:hover:bg-green-800 transition">
                    إضافة عقد جديد
                </button>
            </div>
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach ($contractTypes as $type)
                            <tr>
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    <div class="flex gap-2">
                                        <button onclick="editContract('{{ $type['key'] }}')"
                                            class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300">
                                            <i class="fas fa-edit"></i>
                                        </button>
                                        <button onclick="previewContract('{{ $type['key'] }}')"
                                            class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <form method="POST" action="{{ route('admin.contracts.toggle', $type['key']) }}"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="maintenance_mode" class="sr-only peer"
                                {{ settings('maintenance_mode', false) ? 'checked' : '' }} onchange="this.form.submit()">
                            <div
                    ...
                    <div class="space-y-4">
                        <div>
                            <label for="contract_name"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">اسم العقد</label>
                        ...
                        <div>
                            <label for="contract_content"
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">محتوى العقد</label>
                        ...
                        <div class="flex items-center">
                            ...
                            <label for="is_active" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">تفعيل هذا
                                العقد</label>
                        </div>
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('admin.backup.restore') }}" enctype="multipart/form-data">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">اختر ملف النسخة
                                الاحتياطية</label>
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" name="delete_existing" id="delete_existing"
                                class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-gray-600 rounded dark:bg-gray-700">
                            <label for="delete_existing" class="ml-2 block text-sm text-gray-700 dark:text-gray-300">حذف
                                البيانات الحالية قبل الاستعادة</label>
                        </div>
                    </div>

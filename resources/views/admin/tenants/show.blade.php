            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="group">
                            <label class="text-sm font-medium text-gray-500 uppercase tracking-wide">{{ __('messages.full_name') }}</label>
                            <div class="mt-2 text-lg font-semibold text-gray-900 group-hover:text-blue-600 transition-colors">
                                {{ $tenant->name }}
                            </div>
                        </div>

                        <div class="group">
                            <label class="text-sm font-medium text-gray-500 uppercase tracking-wide">{{ __('messages.id_number') }}</label>
                            <div class="mt-2 text-lg font-mono text-gray-900 group-hover:text-blue-600 transition-colors">
                                {{ $tenant->id_number ?? '-' }}
                            </div>
                        </div>

                        <div class="group">
                            <label class="text-sm font-medium text-gray-500 uppercase tracking-wide">{{ __('messages.phone') }}</label>
                        ...
                        <div class="group">
                            <label class="text-sm font-medium text-gray-500 uppercase tracking-wide">{{ __('messages.email') }}</label>
                        ...
                        <div class="md:col-span-2">
                            <label class="text-sm font-medium text-gray-500 uppercase tracking-wide">{{ __('messages.notes') }}</label>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <div class="bg-gradient-to-r from-green-50 to-green-100 px-6 py-4 border-b border-green-200">
                    <h2 class="text-xl font-semibold text-green-800 flex items-center gap-2">
                        ...
                        <span class="bg-green-200 text-green-800 text-xs px-2 py-1 rounded-full">{{ $activeContracts->count() }}</span>
                    </h2>
                </div>
                ...
                <div class="p-6">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-4 bg-green-50 rounded-lg border border-green-200 hover:shadow-md transition-shadow">
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <div class="font-semibold text-lg text-gray-900">{{ __('messages.unit') }} {{ $contract->unit->unit_number }}</div>
                            ...
                            <div class="text-right">
                                ...
                                <div class="font-medium text-gray-900">{{ $contract->start_date }} - {{ $contract->end_date }}</div>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b">
                    <h2 class="text-xl font-semibold text-gray-800 flex items-center gap-2">
                        ...
                        <span class="bg-gray-200 text-gray-700 text-xs px-2 py-1 rounded-full">{{ $pastContracts->count() }}</span>
                    </h2>
                </div>
                ...
                <div class="p-6">
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center gap-4 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                            ...
                            <div class="flex-1">
                                <div class="font-medium text-gray-900">{{ __('messages.unit') }} {{ $contract->unit->unit_number }} - {{ $contract->unit->building->name }}</div>
                                <div class="text-sm text-gray-500">{{ $contract->start_date }} - {{ $contract->end_date }}</div>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="text-center">
                        <div class="inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold border {{ $colors[$status] ?? 'bg-gray-100 text-gray-800 border-gray-200' }}">
                    ...
                    <div class="bg-gray-50 rounded-lg p-4">
                        <div class="text-center">
                            ...
                            <div class="text-2xl font-bold {{ $tenant->debt > 0 ? 'text-red-600' : 'text-green-600' }}">
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                        ...
                        <span class="inline-flex items-center gap-1 text-sm font-semibold {{ $tenant->user ? 'text-green-600' : 'text-red-600' }}">
                            @if($tenant->user)
                ...
                <div class="p-6">
                    <div class="text-center space-y-3">
                        ...
                        <div class="pt-2 border-t">
                            <div class="flex justify-between items-center">
                                <span class="text-sm font-medium text-gray-800">{{ __('messages.total_units') }}</span>
                                <span class="font-bold text-blue-600">{{ $activeContracts->count() + $pastContracts->count() }}</span>
                            </div>
                        </div>
                    </div>
                </div>

        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
                ...
                <div class="flex items-center space-x-4 rtl:space-x-reverse animate-slide-up">
                    ...
                    <a href="{{ url('/') }}" class="flex items-center space-x-3 rtl:space-x-reverse group">
                        <div class="flex-shrink-0">
                            <div
                                class="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-lg logo-bounce">
                                <!-- Default Logo Icon -->
            ...
            <div class="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <button onclick="toggleDarkMode()"
                    ...
                    <i
                        class="fas fa-sun hidden dark:block text-sm sm:text-base text-yellow-300 group-hover:animate-pulse"></i>
                </button>
                ...
                <div x-data="{ open: false }" class="relative z-[110]" @click.away="open = false">
                    ...
                    <div id="languageDropdown"
                        class="language-dropdown absolute right-0 rtl:left-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden z-50">
                        <div class="py-2">
                            <!-- Arabic -->
                            <a href="{{ route('lang.switch', ['lang' => 'ar', 'redirect' => url()->full()]) }}"
                                class="language-item flex items-center px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 font-bold">
                            ...
                            <a href="{{ route('lang.switch', ['lang' => 'en', 'redirect' => url()->full()]) }}"
                                class="language-item flex items-center px-4 py-3 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                                <span class="text-lg mr-3 rtl:ml-3">🇬🇧</span>
                                <div class="flex flex-col">
    ...
    <div class="shortcuts-grid">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-6">
                {{ __('messages.shortcuts_dashboard_subtitle') }}</h2>
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                @can('create payments')
                    <a href="{{ route('admin.payments.create') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.add_payment') }}</span>
                    </a>
                @endcan
                <!-- Add Expense -->
                @can('create expenses')
                    <a href="{{ route('admin.expenses.create') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.add_expense') }}</span>
                    </a>
                @endcan
                <!-- units.available -->
                @can('view available units')
                    <a href="{{ route('units.available') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.available_rooms') }}</span>
                    </a>
                @endcan
                <!-- Book Room -->
                @can('create bookings')
                    <a href="{{ route('admin.bookings.create') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.book_room') }}</span>
                    </a>
                @endcan
                <!-- My Bookings -->
                @can('view bookings')
                    <a href="{{ route('admin.bookings.index', ['my' => true]) }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.my_bookings') }}</span>
                    </a>
                @endcan
                <!-- Cleaning Dashboard -->
                @can('view cleaning dashboard')
                    <a href="{{ route('admin.cleaning.dashboard') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.cleaning_dashboard') }}</span>
                    </a>
                @endcan
                <!-- maintenance -->
                @role('Maintenance Supervisor')
                    <a href="{{ route('admin.maintenance_requests.index') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.maintenance') }}</span>
                    </a>
                @endrole
                ...
                @role('technician')
                    <a href="{{ route('admin.technician.maintenance') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.my_order_maintenance') }}</span>
                    </a>
                @endrole
                ...
                @can('create maintenance requests')
                    <a href="{{ route('admin.maintenance_requests.create') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.add_maintenance_request') }}</span>
                    </a>
                @endcan
                ...
                @can('view available units')
                    <a href="{{ url('/admin/units-available-text') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.available_text_rooms') }}</span>
                    </a>
                @endcan
                <!-- utilities -->
                @can('view utilities')
                    <a href="{{ route('admin.building-utilities.index') }}"
                        ...
                        <span
                            class="text-sm font-medium text-gray-700 dark:text-gray-300 text-center">{{ __('messages.utilities') }}</span>
                    </a>
                @endcan
            </div>
        </div>

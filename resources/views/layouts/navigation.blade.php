        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16 lg:h-18">
                ...
                <div class="flex items-center gap-4 rtl:flex-row-reverse min-w-0">
                    <a href="{{ url('/') }}"
                        class="flex items-center gap-3 group hover:scale-105 transition-transform duration-300">
                        @if (settings()->app_logo)
                            <div class="relative">
                                <img src="{{ asset('storage/' . settings()->app_logo) }}" alt="Logo"
                                    class="h-10 w-10 rounded-xl shadow-md group-hover:shadow-lg transition-shadow duration-300">
                            </div>
                        @else
                            <div
                                class="h-10 w-10 rounded-xl bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center shadow-md">
                    ...
                    @foreach ($navLinks as $index => $link)
                        @if (isset($link['dropdown']))
                            ...
                            <div x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false"
                                class="relative group" style="z-index: 9999">
                                <button
                                ...
                                <div x-show="open" x-transition:enter="transition ease-out duration-200"
                                    ...
                                    style="z-index: 9999">
                                    <div
                                        class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 backdrop-blur-xl overflow-hidden">
                                        <div class="p-2 space-y-1">
                                            @foreach ($link['dropdown'] as $dropdownLink)
                                                @php
                                                    ...
                                                    $isActive =
                                                        $isNamedRoute &&
                                                        request()->routeIs($dropdownLink['route'] . '*');
                                                @endphp
                                                ...
                                                <a href="{{ $url }}"
                                                    ...
                                                    @if ($isActive)
                                                        <div class="w-2 h-2 rounded-full bg-blue-600 dark:bg-blue-400">
                                                        </div>
                                                    @endif
                                                </a>
                                            @endforeach
                    ...
                    <div class="relative" x-data='@json(['open' => false, 'hasNew' => $unreadNotificationsCount > 0])'>
                            ...
                            @if ($unreadNotificationsCount > 0)
                                <div class="absolute -top-1 -right-1 flex items-center justify-center">
                                    ...
                                    <div class="relative h-3 w-3 bg-red-500 rounded-full flex items-center justify-center">
                                        ...
                                        @else
                                            <span
                                                class="text-[8px] text-white font-bold">{{ $unreadNotificationsCount }}</span>
                                        @endif
                                    </div>
                                </div>
                            @endif
                        ...
                        <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200"
                            ...
                            <div
                                ...
                                {{-- رأس الإشعارات --}}
                                <div
                                    class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20">
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                                        {{ __('messages.notifications') }}</h3>
                                    @if ($unreadNotificationsCount > 0)
                                        <p class="text-sm text-blue-600 dark:text-blue-400">{{ $unreadNotificationsCount }}
                                            {{ __('messages.new_notifications') }}</p>
                                    @endif
                                </div>
                                    ...
                                    @forelse($recentNotifications as $notification)
                                        <a href="{{ $notification->data['url'] ?? '#' }}"
                                            ...
                                            <div class="flex items-start gap-4">
                                                <div
                                                    class="flex-shrink-0 p-2 rounded-xl {{ $notification->read_at ? 'bg-gray-100 dark:bg-gray-700' : 'bg-blue-100 dark:bg-blue-900/30' }}">
                                                    <x-dynamic-component :component="$notification->data['icon'] ??
                                                ...
                                                @if (!$notification->read_at)
                                                    <div
                                                        class="w-2 h-2 rounded-full bg-blue-600 dark:bg-blue-400 flex-shrink-0 mt-2">
                                                    </div>
                                                @endif
                                            </div>
                                        </a>
                                ...
                                {{-- أسفل الإشعارات --}}
                                <div
                                    class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 flex justify-between items-center">
                                    ...
                                    <a href="{{ route('admin.notifications.index') }}"
                                        class="text-sm text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white font-medium transition-colors duration-300">
                ...
                <div class="flex items-center gap-2 sm:hidden">
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open"
                            ...
                            @if ($unreadNotificationsCount > 0)
                                <div
                                    ...
                                    <span
                                        class="text-[8px] text-white font-bold">{{ $unreadNotificationsCount > 9 ? '9+' : $unreadNotificationsCount }}</span>
                                </div>
                            @endif
                        </button>
                        ...
                        <div x-show="open" @click.away="open = false" x-transition
                            ...
                            <div
                                class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200/50 dark:border-gray-700/50 overflow-hidden">
                                <div
                                    class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20">
                    ...
                    <button @click="open = !open"
                        ...
                        <svg class="h-5 w-5 text-gray-600 dark:text-gray-300" stroke="currentColor" fill="none"
                            viewBox="0 0 24 24">
                                ...
                                <div class="pl-6 space-y-1">
                                    @foreach ($link['dropdown'] as $dropdownLink)
                                        @php
                                            ...
                                            $url = $isNamedRoute ? route($dropdownLink['route']) : '#';
                                            $isActive =
                                                $isNamedRoute && request()->routeIs($dropdownLink['route'] . '*');
                                        @endphp
                                        ...
                                        <a href="{{ $url }}" @click="open = false"
                                            ...
                                            @if ($isActive)
                                                <div class="w-1.5 h-1.5 rounded-full bg-blue-600 dark:bg-blue-400 ml-auto">
                                                </div>
                                            @endif
                                        </a>
                                    @endforeach
